...
void Application::HandleUpdate()
{
	...
	if( m_cameraState != CAMERA_PLANE && m_cameraState != CAMERA_GUN )
	{
		...
		if	(m_shadowCastingLightPosition.y < 8.f)
			m_shadowCastingLightPosition.y = 8.f;
	...
	ImGui::Begin("Shadows Tutorial", NULL);
	...
	if (ImGui::CollapsingHeader("Demo Description:", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		ImGui::Text("the several practicals, this one  ");
		ImGui::Text("focused on using hierarchy's aeroplane");
		ImGui::Text("and lighting to create shadows for the");
		ImGui::Text("plane");
		ImGui::Text("");
	}
	...
	if (ImGui::CollapsingHeader("The Demo's control's"))
	{
		...
		case CAMERA_GUN:
			...
			break;
		case CAMERA_LIGHT:
			ImGui::Text("The current camera state is: Light");
			break;
		...
		ImGui::BulletText("Q/A - Zoom in/Zoom out");
		ImGui::BulletText("N/M - Light +/- x");
		ImGui::BulletText("Up dir/Dow dir - Light +/- y");
		ImGui::BulletText("Left dir/Right dir - Light +/- z");
	}
...
void Application::RenderShadow()
{
	...
	XMVECTOR vPlaneRad = XMLoadFloat(&AEROPLANE_RADIUS);
	XMVECTOR vLightPos = XMLoadFloat3(&m_shadowCastingLightPosition);
	//*************************************************************************
	// Your code to adjust the perspective projection of the light goes here
	// You wil need to calculate fovy, zn and zf instead of using these default val

	//radius = opposite
	//lightPos - planePos = adjacent
	//opposite over adjacent
	//tan theta = opposite / adjacent
	//fovy == inverse tan (opposite/adjacent)
	//length = vLightPos - vPlanePos
	float length = XMVectorGetX(XMVector3Length(vLightPos - vPlanePos));

	//need to fix when the light is too close to the plane it turns the shadow off
	//check sphere pos and ensure it doesnt go below zero
		//enforce back to value just before it goes out
			//make sure none of the variables go below 0


	float fovy = atan(AEROPLANE_RADIUS / length) * 2;
	float zn = length - AEROPLANE_RADIUS;
	float zf = length + AEROPLANE_RADIUS;
